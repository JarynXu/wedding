...
输入框右下角显示“AI 写祝福”，已有文字时显示“AI 润色”。可使用时有间歇扫光，生成和冷却期间停止提醒；减少动态效果时保持静态。服务端复用游戏的 AI 配置，客户端只提交祝福原稿和主题。结果填回草稿，宾客点击“送出祝福”后才进入祝福簿；等待过程中宾客修改了原稿时，弹窗询问是否替换。

空白创作轮换日常相伴、彼此支持、一起探索、举杯祝愿等切入点，并变化句式；中式主题不限定为成语贺词。每个服务实例在内存中保留最近 12 份空白创作结果供避重，套话开头或重复开头最多重写一次。润色沿用原稿心意，不参与切入点轮换，也不会把宾客原稿加入这个避重记录。

`POST /api/blessings/polish` 使用独立请求标识和记录表，相同请求可读取已完成结果。浏览器默认间隔 20 秒、每小时最多 20 次；来源网络每分钟 60 次、活动每分钟 120 次，不消耗普通祝福的发送名额。润色中不接收并发重复请求；结果与指纹保留供短期重试，下次申请会清理本活动 2 小时前的辅助记录。草稿原文不写入数据库，成功结果也不作为公开祝福展示。

底部浮条使用连续的毛玻璃底，默认主题增加金色对戒。烟花使用主题颜色的细光尾与发光祝福字：默认主题为香槟金、玫瑰粉和 Love，中式为暖金和囍；光晕使用缓存位图，仍受右侧画布边界约束。礼物旁提示长按，完成长按后收起。最后一页的游戏入口位于右下角、祝福浮条上方，金色双描边、间歇扫光和箭头位移提示可点击；点击区域保持静止。新素材来源与生成提示见 `design/gift-rings-generation.json`。

清理会提升本场 generation 并断开旧订阅。客户端重新同步时清除旧待发请求，刷新页面。清理步骤与连接预算见 [上线与运行维护](operations.md)。

## 署名与 AI 冷却

已认证的称呼显示为署名行，隐藏重复的名字输入；退出认证后保留已记住的称呼供填写。AI 润色的请求忙碌与冷却分别维护。默认每客户端至少间隔 20 秒、每小时最多 20 次，配置为 BLESSINGS_AI_COOLDOWN_SECONDS 和 BLESSINGS_AI_HOURLY_LIMIT。全场每分钟 120 次、来源网络每分钟 60 次的限额保留。限额在共享数据库事务内核对，不因切换服务实例重置。

按钮在请求期间显示等待状态，取得结果后显示冷却倒计时与恢复进度；等待期限保存在本机，429 的 Retry-After 会延长期限。冷却只限制 AI，手动祝福和礼物不受影响。客户端编号属于浏览器标识，不能替代实名身份；来源网络与全场限额继续限制批量重建标识的请求。

页签采用对称留白，整组相对面板居中，关闭按钮有独立点击空间。内部滚动区向面板边缘延展，正文宽度保留，滚动条与时间及正文之间有间距。
