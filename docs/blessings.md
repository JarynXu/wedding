...
输入框右下角在空白时显示“AI 写一句”，已有文字时显示“AI 润色”。服务端复用游戏的 AI 配置，只接收祝福原稿和主题。结果填回草稿，宾客点击“送出祝福”后才进入祝福簿；等待过程中宾客修改了原稿时，弹窗询问是否替换。
...
候选礼物整行使用毛玻璃底，默认主题增加金色对戒。彩色烟花增加发射轨迹、渐隐拖尾与亮点，仍受右侧画布边界约束。最后一页的游戏入口位于右下角，提示气泡说明用途。新素材来源与生成提示见 `design/gift-rings-generation.json`。
