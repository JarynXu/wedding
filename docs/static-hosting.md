...
```text
assets/   构建后的脚本、样式、图片、字体和音乐
share/    两个主题的分享缩略图
vendor/   图形验证码浏览器 SDK
```

压缩上述三个目录，ZIP 内第一层应为 `assets/`、`share/`、`vendor/`。不要把 `static-upload` 或构建指纹目录包在 ZIP 第一层。对应的 `.manifest.json` 位于资源目录外，记录版本、文件路径、字节数和 SHA-256；它用于核对上传结果。
...
重启实例后生效，不需重新构建代码。地址必须对应三个资源目录的父目录，支持子路径。正式地址使用 HTTPS，不接受临时签名参数、账号密码或片段。
