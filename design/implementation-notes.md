...
加载页按背景、花瓣画布、中央信笺的顺序叠放，退出动画由各层承担。弹窗遮罩使用半透明底色，模糊由卡片承担，避免整个背景中的花瓣同时失去清晰度。加载页信笺、日历和地图面板使用 62%–74% 不透明度的底色与 10px 背景模糊，减少背景对阅读的干扰；不支持背景模糊时使用 86% 不透明底色。信息块与次要按钮增加底色覆盖，迎宾牌保留原有材质。
