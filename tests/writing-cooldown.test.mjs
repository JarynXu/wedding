...
test('AI按钮结束请求后保留冷却，重建页面恢复剩余时间，冷却期间不能调用',()=>{
  ...
  try{
    assert.equal(first.button.textContent,'✧ AI 润色');first.wait(20);first.setBusy(true);assert.equal(first.button.dataset.state,'writing');
    ...
    now+=60001;restored.refresh();assert.equal(restored.button.disabled,false);assert.equal(restored.button.textContent,'✧ AI 润色');
