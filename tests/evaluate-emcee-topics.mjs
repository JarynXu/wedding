...
try{
  ...
  const questions=[['婚礼仪式在什么时候举行？','2026年10月17日11:58','10月17日',0],['婚礼在哪举行？','松风礼堂','松风礼堂',1],['新郎的名字是什么？','周明朗','周明朗',2],['新娘的名字是什么？','林一宁','林一宁',3],['宾客几点入场？','11:30','十一点半',4],['喜宴几点开席？','12:28','十二点二十八分',5]];
  ...
  for(const question of event.config.questions)await f.pool.query('UPDATE wedding_game_question_voice SET deck=$3 WHERE room_id=$1 AND question_id=$2 AND config_version=3',[f.room,question.id,{phrasings:[question.title],hints:[],choices:[],source:'evaluation'}]);
  ...
  for(let index=0;index<questions.length;index++){
    const [title,,input,score]=questions[index];await f.service.conversation.enqueue(participantId,{requestId:randomUUID(),text:input});
    ...
    const pass=noFutureClaim&&me.participant.score===score&&reply.questionText===(questions[index+1]?.[0]||null)&&(!questions[index+1]||!reaction.includes(questions[index+1][1]));
  ...
  await writeFile('docs/emcee-topic-evaluation.json',JSON.stringify(report,null,2)+'\n');if(report.passed!==report.total)process.exitCode=1;
