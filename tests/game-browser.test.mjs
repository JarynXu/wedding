...
test('宾客手机验证码、六题、榜单与兑奖凭据，使用隔离短信替身',{skip:!gameTestDatabase&&'需要隔离 PostgreSQL',timeout:90000},async()=>{
  ...
  try {
    ...
    for(const [theme,width,index] of [['classic',390,1],['chinese',320,2]]){
      ...
      await page.locator('#gameAnswer').fill('第一份隔离测试回答');await page.getByRole('button',{name:'发送回答',exact:true}).click();
      await page.locator('dialog[open]').waitFor();assert.match(await page.locator('.dialog-content').innerText(),/提交结果待确认/);await page.locator('.dialog-primary').click();
      ...
      for(let q=2;q<=6;q++){
        ...
        await page.locator('#gameAnswer').fill(`第${q}份隔离测试回答`);await page.getByRole('button',{name:'发送回答',exact:true}).click();await page.locator('.game-my-answer').waitFor();
      }
      ...
      if(process.env.WEDDING_QA_DIR)await page.screenshot({path:join(process.env.WEDDING_QA_DIR,`game-${theme}-score.png`),fullPage:true,animations:'disabled'});
    ...
    await winner.locator('.game-score').waitFor();await winner.locator('.game-claim-code').waitFor({timeout:12000});
