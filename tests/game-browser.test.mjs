...
test('宾客手机验证码、六题、榜单与兑奖凭据，使用隔离短信替身',{skip:!gameTestDatabase&&'需要隔离 PostgreSQL',timeout:90000},async()=>{
  ...
  try {
    ...
    for(const [theme,width,index] of [['classic',390,1],['chinese',320,2]]){
      ...
      await page.goto(origin+'/game.html?theme='+theme);await page.locator('dialog[data-kind=game-intro][open]').waitFor();assert.equal(await page.locator('#gameLogin').count(),0);await page.getByRole('button',{name:'我来试试',exact:true}).click();await page.locator('#gameLogin').waitFor();
      ...
      await page.locator('#gameMenuToggle').click();await page.locator('[data-game-tab=board]').click();await page.locator('.game-leaderboard li').first().waitFor();assert.equal(await page.locator('#gameViewTitle').innerText(),'默契榜');assert.equal(await page.locator('#gameBoardPhase,#gameLiveRank').count(),0);assert.equal(await page.locator('.game-personal-prize strong').innerText(),index===1?'大号毛绒玩偶':'中号毛绒玩偶');if(process.env.WEDDING_QA_DIR)await page.screenshot({path:join(process.env.WEDDING_QA_DIR,'prize-board-'+theme+'.png'),animations:'disabled'});await page.locator('#backToInvitation').click();await page.locator('#gameAnswer').waitFor();
