...
test('宾客手机验证码、六题、榜单与兑奖凭据，使用隔离短信替身',{skip:!gameTestDatabase&&'需要隔离 PostgreSQL',timeout:90000},async()=>{
  ...
  try {
    ...
    for(const [theme,width,index] of [['classic',390,1],['chinese',320,2]]){
      ...
      await page.locator('#gameMenuToggle').click();await page.locator('[data-game-tab=board]').click();await page.locator('.game-leaderboard li').first().waitFor();assert.match(await page.locator('.game-board-note').innerText(),/当前默契榜/);
