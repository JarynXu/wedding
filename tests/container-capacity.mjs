...
const f = await gameFixture(), directory = await mkdtemp(resolve('.temp/reports/capacity-')), containers = [], ports = [], streams = [], logs = [];
...
const staticApp = express(); staticApp.use((_req, res, next) => { res.set('Access-Control-Allow-Origin', '*'); next(); }); staticApp.use(express.static(resolve('public')));
...
async function consume(path, cookie, body) {
  ...
  for await (const chunk of response.body) { if (isAsset) staticBytes += chunk.length; if (!external) bytes += chunk.length; }
}
...
try {
  ...
  const paths = ['/', '/game.html', '/music/classic/01-Close%20to%20You-Olivia%20Ong.mp3', '/assets/classic/portrait.webp', '/assets/classic/time.jpg', '/assets/classic/location.jpg', '/assets/classic/closing.jpg', '/assets/shared/rose-petals.webp', '/assets/shared/prize-plush.webp', ...patterns.map(pattern => { const file = assets.find(file => new RegExp(pattern).test(file)); assert.ok(file, pattern); return '/app/' + encodeURIComponent(file); })];
  ...
  await writeFile(process.env.CAPACITY_REPORT || join(directory, 'report.json'), JSON.stringify(report, null, 2) + '\n');
