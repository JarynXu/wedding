...
export class BlessingWriter {
  constructor(config){this.configured=Boolean(config);this.config=config;this.client=config?new JsonModelClient(config):null;this.controller=new AbortController();}
  async compose(text,theme){
    if(!this.client)throw new BlessingError('AI_UNAVAILABLE','AI写祝福正在准备中',503);
    try{
      const response=await this.client.complete({model:this.config.hostModel||this.config.model,temperature:.7,schema,messages:[
