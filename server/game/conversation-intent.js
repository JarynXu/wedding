...
const intents=['answer','chat','pause','score','standing','hint','options','repeat','continue','skip','rules','wedding','injection','clarify'];
...
score：询问自己答对几题、成绩、进度；standing询问是否达标、能否获奖或奖品状态。查询不等于要求修改。hint要提示；options要备选项；repeat希望换个问法；continue希望继续或开始；skip明确放弃当前题；rules问玩法、奖项规则、截止时间、兑奖或登录说明；wedding：询问婚礼日期、地点、日程、新人姓名等公开请柬信息。询问婚礼事实不是作答；chat正常聊天、打招呼、紧张、调侃、询问身份。pause明确说暂时不玩、稍后再答、想安静想一想；这不是放弃题目，不计为skip。闲聊不是答错。“不会”“想不起来”先按hint或clarify处理，明确说跳过才用skip；轻松调侃“放点水”不等于要求改分。
