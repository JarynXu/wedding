...
export class QuestionDeck {
  constructor(config,judge,client){this.config=config;this.client=client||(config?new JsonModelClient(config):null);this.judge=judge;}
  async prepare(question,instructions,signal){
    if(!this.client)return fallbackDeck(question);
    try{
      const response=await this.client.complete({operation:'preparing',model:this.config.model,schema,temperature:.6,messages:[
      ...
      const verification=await this.client.complete({operation:'preparing',model:this.config.model,schema:validationSchema,messages:[
